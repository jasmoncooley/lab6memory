...
void list_add_ascending_by_address(list_t *l, block_t *newBlock){
   ...
   */
    node_t *current;
    node_t *prev;
    node_t *newNode = node_alloc(newBlock);
    ...
    if(l->head == NULL){
      ...
      if(current->next == NULL) {  //only one node in list
         if(newBlock->start <= current->blk->start) {  
            newNode->next = l->head;
            l->head = newNode;
         }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newBlock){
   /*
   ...
   */
    node_t *current;
    node_t *prev;
    node_t *newNode = node_alloc(neBlock);
    int newBlock_size = newNode->blk->end - newNode->blk->start;
    int curr_blocksize;
    ...
    if(l->head == NULL){
      ...
      if(current->next == NULL) {  //only one node in list
         if(newBlock_size >= curr_blocksize) {  // place in front of current node
            newNode->next = l->head;
            l->head = newNode;
         }
         ...
         if(newBlock_size >= curr_blocksize) {  // place in front of current node
            ...
            while(current != NULL && newBlock_size <= curr_blocksize) {
                 prev = current;
                 current = current->next;
...
void list_add_descending_by_blocksize(list_t *l, block_t *blk){
  ...
  node_t *prev;
  node_t *newNode = node_alloc(blk);
  int newBlock_size = blk->end - blk->start;
  int curblk_size;
  ...
  if(l->head == NULL){
    ...
    if(current->next == NULL) {  //only one node in list
       if(newBlock_size >= curblk_size) {  // place in front of current node
          newNode->next = l->head;
          l->head = newNode;
       }
       else {   // place behind current node
       ...
       if(newBlock_size >= curblk_size) {  // place in front of current node
          ...
          while(current != NULL && newBlock_size <= curblk_size) {
               prev = current;
               current = current->next;
